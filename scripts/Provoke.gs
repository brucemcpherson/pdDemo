  ...
  ns.run = function (namespace,method) {
    ...
    return new Promise(function ( resolve , reject ) {
      ...
      .withSuccessHandler (function(result) {
        // since I may be using 'faux promises' on the server side, we'll clean that up
        if (result.hasOwnProperty ("then") && result.hasOwnProperty ("_value")) result = result._value;
        resolve (result);
      })
